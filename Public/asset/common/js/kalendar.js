...
/***************************/
	...
	dateDay.on('click', function(){
		var t_year = $('#year').find($('select[name=year]')).val();
		var t_month = eval(parseInt($('#month').find($('select[name=year]')).val()) + 1);
		var t_day = $(this).text();
		var t_date = t_year + '-' + t_month + '-' + t_day;
		var t = true;
		if(t_day == '' || t_day == null || t_day == false){
			t = false;
		}
		console.log(t_date);
		//循环所有值是否存在
		var time = $('#no_time > a.no_t');
		$.each(time, function(i){
			d = $('#no_time > a.no_t')[i].text;
			if(t_date == d){
				t = false;
				return false;
			}
		...
		if(t){
			$('#no_time').append('<a href="javascript:0;" class="btn btn-sm btn-success no_t">'+ t_date +'</a>');
			//隐藏表单传值
			var r = $('input[name=invalid_time]').val();
			$('input[name=invalid_time]').val( r + ',' + t_date);
		}
	})
