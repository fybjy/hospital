...
function wechatAPI() {
	...
	this.configInit = function () {
		...
		//初始化分享配置
		$.ajax({
			url: 'http://www.weiwubao.com/ajax/mp/jsapi_init/',
			dataType: 'json',
			method: 'get',
			data: {mpid: this.mpid},
	...
	this.uploadVoice = function (localId, callback) {
		...
		wx.uploadVoice({
			...
			success: function (res) {
				...
				//AJAX下载
				$.ajax({
					url: 'http://www.weiwubao.com/ajax/mp/download_media/',
					dataType: 'json',
					method: 'get',
	...
	this.shareInit = function () {
		...
		wx.ready(function () {
			...
			wx.hideMenuItems({
				...
				menuList: ["menuItem:setFont", "menuItem:openWithSafari", "menuItem:originPage"] // 要隐藏的菜单项，所有menu项见附录3
			});
			//分享给好友
