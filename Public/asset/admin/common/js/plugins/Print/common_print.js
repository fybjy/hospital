...
function Prints(data) {
    ...
    if(data.title == "但愿人长久- 中国唐宋名篇音乐朗诵会"){
        // title = "紫御温泉•但愿人长久";
        title = title.replace('data_title', '&nbsp;&nbsp;紫御温泉•但愿人长久<br>中国唐宋名篇音乐朗诵会');
    }else{
    ...
    if (data.host != '') {
        data.hosts = "中共吉林市委宣传部&nbsp;&nbsp;吉林市文化广电新闻出版局";
        // content = content.replace('data_host', '<p style="line-height: 4.5mm; font-size: 3.0mm;margin: 0;">主办：' + data.host + '</p>');
        content = content.replace('data_host', '<p style="line-height: 4.5mm; font-size: 3.0mm;margin: 0;">主办：中共吉林市委宣传部&nbsp;&nbsp;吉林市文化广电新闻出版局</p>');
    } else {
        content = content.replace('data_host', '');
    }

    if (data.undertake != '') {
        content = content.replace('data_undertake', '<p style="line-height: 4.5mm; font-size: 3.0mm;margin: 0;">承办：' + data.undertake + '</p>');
    } else {
        content = content.replace('data_undertake', '');
    }
    content = content.replace('data_ad', '<p style="line-height: 4.5mm;font-size: 3.0mm;margin: 0;">协办：江城日报社&nbsp;&nbsp;吉林市广播电视台&nbsp;&nbsp;吉林市人民大剧院</p>');
    ...
    try {
        ...
        LODOP.ADD_PRINT_HTM('18mm', '40mm', '80mm', '16mm', title);
